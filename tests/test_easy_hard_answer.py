...
def test_merge_train_validation() -> None:
    train = pd.DataFrame({
        "s0": ["a", "b", "c", "q"],
        "p0": ["d", "e", "f", "r"],
        "o0_targets": [set(["g", "h"]), set(["i", "j", "k"]), set(["l"]), {"s"}]
    })
    validation = pd.DataFrame({
        "s0": ["b", "c", "m", "q"],
        "p0": ["e", "f", "n", "r"],
        "o0_targets": [set(["l", "k"]), set(["l", "p"]), set(["o"]), {"s"}]
    ...
    expected_result = _stringify_sets(pd.DataFrame({
        "s0": ["b", "c", "m"],
        "p0": ["e", "f", "n"],
        "targets-easy": [{"i", "j", "k"}, {"l"}, {}],
        "o0_targets-hard": [{"l"}, {"p"}, {"o"}]
    }), target_column)
...
def _stringify_sets(merged: pd.DataFrame, target_column: str) -> pd.DataFrame:
    ...
    merged["targets-easy"] = merged["targets-easy"].map(lambda easyset: "|".join(sorted(list(easyset))) if isinstance(easyset, set) else "")
    return merged
