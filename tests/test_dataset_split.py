...
def split_n_equal_hash(input_file: pathlib.Path, n: int, tmp_path: pathlib.Path):
    fraction = 1.0 / n
    splits = [dataset_split.Split(fraction, tmp_path / f"split{i}") for i in range(n)]
    dataset_split.split_hash(input_file, splits, seed=0)
    return splits


def test_one_hash_split(tmp_path):
    """Test 'splitting' all into the same split by hashing'."""
    input_file = tmp_path / "input.txt"
    splits = split_n_equal_hash(input_file, 1, tmp_path)
    data = get_split(splits[0])
    assert len(data) == 10000


def test_three_equal_hash_splits(tmp_path):
    """Test splitting all into three splits by hashing'."""
    input_file = tmp_path / "input.txt"
    splits = split_n_equal_hash(input_file, 3, tmp_path)
    data = get_split(splits[0])
    assert len(data) == 3279
    data = get_split(splits[1])
    assert len(data) == 3375
    data = get_split(splits[2])
    assert len(data) == 3346


def test_three_unbalanced_hash_splits(tmp_path):
    input_file = tmp_path / "input.txt"
    splits = [dataset_split.Split(0.2, tmp_path / "20"),
              dataset_split.Split(0.1, tmp_path / "10"),
              dataset_split.Split(0.7, tmp_path / "70")]
    dataset_split.split_hash(input_file, splits, seed=0)
    assert len(get_split(splits[0])) == 1916
    assert len(get_split(splits[1])) == 1012
    assert len(get_split(splits[2])) == 7072
