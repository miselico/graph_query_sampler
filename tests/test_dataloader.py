...
def _generate_graph(
    ...
    easy_targets = torch.as_tensor(data=list(set(random.randrange(max_entity_id) for _ in range(random.randrange(2, 6)))))
    hard_targets = torch.as_tensor(data=list(set(random.randrange(max_entity_id) for _ in range(random.randrange(2, 6)))))
    return TorchQuery(
...
def test_collation(tmp_path: pathlib.Path) -> None:
...
#     assert isinstance(qualifier_index, torch.Tensor)
...
#     assert isinstance(data_geometric.query_diameter, torch.Tensor)
