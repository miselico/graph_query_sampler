...
def read_queries_from_proto(
    ...
    query_sturecture: str,
) -> Iterable[TorchQuery]:
    """Yield query data from a protobuf."""
    assert not (reify and remove_qualifiers), "cannot both reify and remove qualifiers"
    if reify:
        yield from read_queries_from_proto_with_reification(input_path, dataset, query_sturecture)
    else:
        yield from read_queries_from_proto_without_reification(input_path, remove_qualifiers, query_sturecture)
...
def read_queries_from_proto_with_reification(
    ...
    for query in qd.queries:
        ...
        # set structure
        b.set_structure(query_sturecture)
